...
export function Home() {
  ...
  const getAllOrders = async () => {
    try {
      ...
      setTotalSales(totalAmountSum.toFixed(2));
      ...
      // Ensure all amounts are rounded to two decimal places before setting the state
      const roundedMonthlySales = monthlySales.map((amount) => parseFloat(amount.toFixed(2)));
      setSalesAmountArray(roundedMonthlySales);
                ...
                {orders?.slice(0, 10)?.map((item, index) => {
                  return (
                    <tr key={index} className="">
                      ...
                      <td>
                        <p
                          className={`text-center ${item.orderStatus.toLowerCase() === "pending"
